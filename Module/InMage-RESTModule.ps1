...
<FunctionRequest Name="InstallAgent" Include="No">      
    <ParameterGroup Id="InstallAgentList1">
        ...
        <Parameter Name="CXPort" Value="80"/>		
    </ParameterGroup>
</FunctionRequest> 
</Body> 
</Request>
...
######################################Code section to build the XML 
        ...
        $Temp3 = $GetAgentInstallationXML.Clone()
        ...
        $Temp3.Request.Header.Authentication.CXUserName = $CXUserName
        $Temp3.Request.Header.Authentication.CXPassword = $CXPassword
        ...
        if ($AuthMethod -eq "CXAuth")
        {
            ...
            $Temp3.Request.Header.Authentication.AuthMethod = $AuthMethod
        ...
        ##############################populate required parameters for Request Body
            ...
            else
            {
                ...
                $PushServerID = ($XML.Response.Body.Function.FunctionResponse.ParameterGroup[0].Parameter | Where-Object {$_.Name -eq "PushServerId"}).Value
...
#region
            ...
            {
                Write-Verbose "Installation started on HostIP : $HostIP, with message $($ResponseValue[0].Message)"
                ...
                {
                    $status = Get-InMageAgentInstallationStatus -HostIP $HostIP
                    If ( ($status.Message -eq "Failed")  -or (($status.Message -eq "Invalid")) )
                    {
                 ...
                 if (($secondCounter.elapsed -gt $Threshold) -and ($status.Message -ne "Completed") )
                 {
                    if ($status.Message -eq  "Pending")
                    {
                        $Object.ErrorCode = 9
                        $Object.Message  = "Installation is still pending on Target IP."
                        
                    }
                    if ($status.Message -eq  "InProgress")
                    {
                        $Object.ErrorCode = 9
                        $Object.Message  = "Installation is still in Progress."
                        
                    }
                    if ($status.Message -eq  "Failed")
                    {
                        $Object.ErrorCode = 11
                        $Object.Message  = "Installation Failed on Target IP."
                        
                    }if ($status.Message -eq  "Invalid")
                    {
                        $Object.ErrorCode = 11
                        $Object.Message  = "Installation State is invalid on Target IP."
                        
                    }    


                    #populate host IP and return object
                    $object.HostIP = $MachineIP
                    return $Object
...
#TODO : remove dependency from Posh-SSH for mapping disk to device ID on master target.
    ...
    #update : Obsolete!!!!!!!!!! 
    #Function Initialize-InMageDiskProtection
    {
        Param
        (
            [parameter(Mandatory=$False)]
        ...
        $RestApiErrorCodes = $ScoutSettings.RestApiErrorCodes
         ...
         <#
            ...
            2 : Error connecting to Source Maching on RPC(Windows)
            3 : Error connecting to Source Maching on SSH(Linux)
            4 : Error Calling Pair API to CS server.
            5 : Unable to connect to Target HyperV Server for VHD creation.
            6 : Unable to SSH to Linux Master Target Server.
            7 : Target VHD Path not found/unable to create
            8 : Insufficient SCSI slots in Master Target
            9 : Master Target not located on specified Hyper-V Target.
            10 : Failed to create VHD on Hyper-V Server.Please perform cleanup
            11 : Failed to attach VHD to Master Target. Please perform Cleanup.
            99 : Generic Error      
...
#region
            ...
            $Object | Add-Member -Type NoteProperty -Name ErrorCode -Value $null
            $Object | Add-Member -Type NoteProperty -Name Message -Value $null
            $Object | Add-Member -Type NoteProperty -Name MachineObject -Value $null     # include object $machineObject declared below.
            $Object | Add-Member -Type NoteProperty -Name ErrorCodeTable -Value ("0 : No Errors","1 : Error retrieving Source details from CS","2 : Error connecting to Source Maching on RPC(Windows)","3 : Error connecting to Source Maching on SSH(Linux)","4 : Error Calling Pair API to CS server.","5 : Unable to connect to Target HyperV Server for VHD creation.","6 : Unable to SSH to Linux Master Target Server.","7 : Target VHD Path not found/unable to create","8 : Insufficient SCSI slots in Master Target.","9 : Master Target not located on specified Hyper-V Target.","10 : Failed to create VHD on Hyper-V Server.Please perform cleanup.","11 : Failed to attach VHD to Master Target. Please perform Cleanup.","99 : Generic Error)")

            #declare Custom objects to store Mapping information to be returned to Main Host.

            #create a master array object for Machine Object
            # Included in $Object
            $MachineObjectArray = @()
            $MachineObject = New-Object PSObject
            $MachineObject | Add-Member -Type NoteProperty -Name InformationTag -Value $null                       # currently can be Disk or Network
            $MachineObject | Add-Member -Type NoteProperty -Name InformationObject -TypeName PSObject -Value @()   # This is an array of $DiskObjects declared below.


            #create a custom object with disk information
            #Inluded in $MachineObject
            $DiskArray = @()
            $DiskObject = New-Object PSObject
            $DiskObject | Add-Member -Type NoteProperty -Name DeviceLabel -Value $null                               
            $DiskObject | Add-Member -Type NoteProperty -Name DeviceName -Value $null
            $DiskObject | Add-Member -Type NoteProperty -Name DeviceID -Value $null
            $DiskObject | Add-Member -Type NoteProperty -Name DiskType -Value $null
            $DiskObject | Add-Member -Type NoteProperty -Name SizeBytes -Value $null
            $DiskObject | Add-Member -Type NoteProperty -Name LogicalSector -Value $null
            $DiskObject | Add-Member -Type NoteProperty -Name PhysicalSector -Value $null
            $DiskObject | Add-Member -Type NoteProperty -Name DiskPartitions -TypeName PSObject -Value @()          #this variable stores any partition details from CS server. Only to be used it partition based backup is being done(windows)
            $DiskObject | Add-Member -Type NoteProperty -Name DiskScsiId -TypeName PSObject -Value @()
            $DiskObject | Add-Member -Type NoteProperty -Name DiskMapping  -Value $null
            $DiskObject | Add-Member -Type NoteProperty -Name DiskMappingVHD  -Value $null
            $DiskObject | Add-Member -Type NoteProperty -Name PairingStatus  -Value $null                           #check on return . If this is #false, perform cleanup.

            #create a custom Partition Object
            # this is required for Windows. May need to add additional params.
            #currently not required as source --> Target is linux
            $PartitionArray = @()
            $PartObject = New-Object PSObject
            $PartObject | Add-Member -Type NoteProperty -Name PartitionLabel -Value $null
            $PartObject | Add-Member -Type NoteProperty -Name ISSystemVolume -Value $null
            $PartObject | Add-Member -Type NoteProperty -Name SizeBytes -Value $null
            $PartObject | Add-Member -Type NoteProperty -Name FilesystemType -Value $null
            $PartObject | Add-Member -Type NoteProperty -Name StartSector -Value $null
            $PartObject | Add-Member -Type NoteProperty -Name EndSector -Value $null
#endregion

############################XML for calling protection API : Do not change or edit.########################################
...
<FunctionRequest Name="CreateProtection" Include="No">
    <Parameter Name="SourceIP" Value="0.0.0.0" />
    <Parameter Name="TargetIP" Value="0.0.0.0" />   
    <Parameter Name="ProcessServerIP" Value="0.0.0.0" />     
    <Parameter Name="RetentionVolume" Value=""/>
...
<Body> 
<FunctionRequest Name="GetProtectionStatus" Include="No">
...
###########################Populate Authentication details in XML
            ...
            if($AUTHMethod -eq "CXAuth")
            {
                ...
                $GetProtectionXML.Request.Header.Authentication.AuthMethod = "CXAuth" 
                ...
                $GetProtectionXML.Request.Header.Authentication.CXPassword = $CXPassword
                $GetProtectionXML.Request.Header.Authentication.CXUserName = $CXUserName
...
#############################Pull Source and Master Target details from InMage Scout#########################################
#region    

            #get Source Details from InMage
            Write-Verbose "Retrieving Source Host information $SourceIP from InMage Server."
            $SourceDetails = Get-InMageHostDetails -SourceIP $SourceIP -ScoutSettings $ScoutSettings

            #check if the InMage call instead returned error code
            if ($SourceDetails.ErrorCode -ne 0)
            {
            $Object.ErrorCode = 99
            $Object.HostIP = $SourceIP
            $Object.Message = "Error retrieving Host details from Scout Server."
            return $Object
            }


                
                #GetTarget VM details from InMage server
                Write-Verbose "Retrieving Target Host information $targetIP from InMage Server."
                $TargetXML = Get-InMageHostDetails -SourceIP $TargetIP -ScoutSettings $ScoutSettings
                if($TargetXML.ErrorCode -ne 0)
                {
                    Write-Error "Specified Machine not registered with InMage Server : $CXServerIP"
                    $Object.HostIP = $SourceIP
                    $Object.ErrorCode = 99
                    $Object.Message = "Target Machine not found in CS Server : $CSServerIP"
                    return $Object
                }
...
#############################Check respective Hyper-V Server WinRM session, Source SSH and Target SSH#########################           
#region            
                #connect to Linux Master Target and create session.
                #Update : not needed any more
                
                <#
                Try
                {
                $TrgSession = New-SSHSession -ComputerName $TargetIP -Credential $TrgCmpCredential -Port $SSHPort -AcceptKey -ConnectionTimeout 30
                
            }
                catch
                {
                write-error "Error connecting to Master Target on SSH port."
                $Object.HostIP = $SourceIP
                $Object.ErrorCode = 6
                $Object.Message = "Error connecting to Master Target Server : $TargetIP on SSH Port : $SSHPort"
                return $Object 
            }
                #>
                #check if Hyper-V Host is still reachable
                try
                {
                $TargetHostPSSession = New-PSSession -Computer $TargetHyperVIP -Credential $TrgHyperVCmpCredential 
                $Object.HostIP = $SourceIP
                $Object.ErrorCode = 0
                $Object.Message = "" 
                }
                ...
                $Object.HostIP = $SourceIP
                $Object.ErrorCode = 5
                $Object.Message = "Error Remoting to Target Hyper-V Server $TargetHyperVIP. `n 
                ErrorMessage : $($error[0].Message)`n
                ErrorStack : $($error[0].StackTrace)"
                Write-Error "Error connecting to Target Hyper-V Server : $TargetHyperVIP"
                }

                #connect to Linux Source OS and create session
                #Update : Not needed any more
                <#try
                {
                    $SrcSession = New-SSHSession -ComputerName $SourceIP -Credential $SrcCmpCredential -Port $SSHPort -AcceptKey -ConnectionTimeout 30
                }
                catch
                {
                    Write-Error "Error connecting to $SourceIP on port $SSHPort."
                    $SrcSession = $null
                    $Object.ErrorCode = 3
                    $Object.Message = "Error connecting to Linux machine $SourceIP on Port : $SSHPort."
                    $Object.HostIP = $SourceIP
                }
                #>
#endregion
...
#region            
##############start process for linux Source machine. 
         if ($OSbuild -eq "Linux")
         {
            
            #List out the disks from SourceXML
            $SourceXML = [xml]($SourceDetails.HostDetailsXML)
            $disks = ($SourceXML.FunctionResponse.ParameterGroup | Where-Object {$_.ID -eq "DiskList"}).Clone()
            $MachineObject.InformationTag = "Disks"
            

            #run through each disk and populate disk information custom object 
            foreach ($disk in $Disks.ParameterGroup)
            {
                $temp = $DiskObject | select *
                $temp.DeviceID = $disk.ID
                $temp.DeviceLabel = ($Disk.Parameter | Where-Object {$_.Name -eq "DiskName"}).Value
                $temp.SizeBytes = ($Disk.Parameter | Where-Object {$_.Name -eq "Size"}).Value
                $temp.DiskType = ($Disk.Parameter | Where-Object {$_.Name -eq "IdeOrScsi"}).Value
                $temp.DiskScsiId = ($Disk.Parameter | Where-Object {$_.Name -eq "ScsiID"}).Value
                $temp.LogicalSector = $LogicalSectorSize
                $temp.PhysicalSector = $PhysicalSectorSize
                #add the disk entries to array and continue loop
                $DiskArray += $temp
            }
                ...
                #check if the MasterTarget VM is on the Hyper-V Target
                #Updated : Script block now relies on MAC address to search out VM name. Ensure Target is registered in InMage Scout.
#################TODO : update script block to reflect if Hyper-V server is a cluster in place of standalone node.
                #region
                    #get Network Adapter details from Retrieved Target InMage Data.
                    $TargetNICDetails = ([xml]($TargetXML.HostDetailsXML)).FunctionResponse.ParameterGroup | Where-Object {$_.ID -eq "MacAddressList"}

                    #Get MAC address of only one of NIC,
                    $TargetMacAddress = (($TargetNICDetails.ParameterGroup).Parameter | Where-Object {$_.Name -eq "MacAddress"}).Value
                    $TargetMacAddress = $TargetMacAddress.replace(":","")    
                    
                $TargetVMname = invoke-command -Session $TargetHostPSSession -ArgumentList ($TargetMacAddress) -ScriptBlock {$MAC = $args[0]; (get-VM | get-VMnetworkAdapter | Where-Object {$_.MacAddress -eq $MAC}).VMName }
                #endregion


                if ( [string]::IsNullOrEmpty($TargetVMname) -eq $null)
                {
                    Write-Error "Error : Master Target not found on specified Hyper-V Host."
                    $Object.HostIP = $SourceIP
                    $Object.ErrorCode = 9
                    $Object.Message = "Master Target not found on Hyper-V."

                    return $Object
                }
            
                #check the number of available SCSI slots on VM.Scout supports only upto 54 disks.
                $SCSIController = Invoke-Command -Session $TargetHostPSSession -ArgumentList $TargetVMname -ScriptBlock {$VMname = $args[0];$VMname | get-VM | Get-VMScsiController  }

                #calculate total  disk slot attached to SCSI controllers.
                $DiskCount = Invoke-Command -Session $TargetHostPSSession -ArgumentList $TargetVMname -ScriptBlock {$VMname = $args[0]; ($VMname | get-VM | Get-VMScsiController | get-VHD).Count  }
                $AvailableSCSIDiskSlots = 256 - $DiskCount                    


                    #check if total available SCSI disks in VM are less than 256 - (required) disks.
                    #if this fails, return with error, Protection cannot proceed
                    if ( $AvailableSCSIDiskSlots -lt $DiskArray.Count )
                    {
                        $Object.ErrorCode = 8
                        $Object.HostIP = $SourceIP
                        $Object.Message = "Insufficient SCSI slot in specified Master Target : $TargetIP"
                        return $Object        
                    }
                ...
                #create disks in specified folder and attach to VM. 
                #This will connect to Hyper-V and create required disks.
                # we are not covering Multipath and logical disks here. Only Physical Disks.
                #TODO : more stringent error checking for allowing cleanup.
                foreach ($viableDisk in $DiskArray)
                {
                    #TODO : Add error control and mapping success.Add reverting back of disk status in case of error.
                    #filter out the mapper disks, these are Multipath disks, we  are not replicating MPIO disks at the moment.
                    if  ($viableDisk.DeviceLabel.contains("mapper") -eq $false )
                    {

                                    #create disk name from disk path
                                    $diskName = ($SourceXML.FunctionResponse.Parameter | Where{$_.Name -eq "HostName"}).Value + "_"  + ([guid]::NewGuid()).Guid  +"_" + $ViableDisk.DeviceID + ".VHDX"                   
                                    $DiskPath = $TargetVHDPath + $diskName
                        
                                    #call WinRM to Hyper-V server to create Disk.
                                    try
                                    {
                                        $VHDObject = Invoke-Command -Session $TargetHostPSSession -ArgumentList ($DiskPath,$ViableDisk) -ScriptBlock {
                                        $DiskPath = $args[0];$viableDisk = $args[1];new-VHD -Path $DiskPath -SizeBytes $viableDisk.SizeBytes -Dynamic -LogicalSectorSizeBytes $ViableDisk.LogicalSector -PhysicalSectorSizeBytes $viabledisk.PhysicalSector
                                        }
                                    }
                                    catch
                                    {
                                        Write-Error "Failed to create Virtual Hard disk $DiskPath on Hyper-V Server : $TargetHostPSSession"
                                        $Object.ErrorCode = 10
                                        $Object.Message = "Failed to create Virtual Hard disk $DiskPath on Hyper-V Server : $TargetHostPSSession.`n
                                        Error Message : ($error[1].Exception)"
                                        
                                        #update the current disk status
                                        $viableDisk.PairingStatus = $false
                        ...
                        #attach disk to Master Target VM
                        
                                    try
                                    {
                                        Invoke-Command -Session $TargetHostPSSession -ArgumentList ($DiskPath,$TargetVMname) -ScriptBlock {$VMname = $args[1];$DiskPath = $args[0]; get-VM -Name $VMname | Add-VMHardDiskDrive -ControllerType SCSI -Path $Diskpath}
                                    }
                                    catch
                                    {
                                        Write-Error "Failed to Attach Virtual Hard disk $DiskPath to MT:$TargetVMname   on HyperV Server  : $TargetHostPSSession"
                                        $Object.ErrorCode = 11
                                        $Object.Message = "Failed to Attach Virtual Hard disk $DiskPath to MT:$TargetVMname   on HyperV Server  : $TargetHostPSSession`n
                                        Error Message : ($error[1].Exception)"
                                    ...
                                    #update The DiskObject
                                    $viableDisk.PairingStatus = $true
                                    $viableDisk.DiskMappingVHD = $DiskPath
                ...
                #perform mapping from Hyper-V Target VHD and MT logical disk.
                #force target Agent update status
                $UpdateStatus = Sync-InMageHostDetails -MachineIP $TargetIP -ScoutSettings $ScoutSettings
                    #TODO : add error handling for Refresh update,based on returned error code
                
                #get Target information
                $TargetXML = [xml](Get-InMageHostDetails -MachineIP $TargetIP -ScoutSettings $ScoutSettings).HostDetailsXML
                $TargetDiskInfo = ($targetXML.FunctionResponse.ParameterGroup[0]).ParameterGroup

                #Calculate DiskUUID and perform matching with Hyper-V and MasterTarget and subsequent Source
                foreach ($viableDisk in $DiskArray)
                {
                   #filter out the mapper disks, these are Multipath disks, we  are not replicating MPIO disks at the moment.
                    if  ($viableDisk.DeviceLabel.contains("mapper") -eq $false )
                    {  
                        $DiskIdentifier = Invoke-Command -Session $TargetHostPSSession -ArgumentList ($viableDisk.DiskMappingVHD,$TargetVMname) -ScriptBlock {
                        $DiskPath = $args[0];
                        $VMName = $args[1];
                        (get-VM $VMName | Get-VMScsiController | get-VHD  | Where-Object {$_.Path -eq $DiskPath}).DiskIdentifier
                        }
                        $DiskUUID = (($DiskIdentifier.split("-"))[4]).ToLower()
                        Write-debug " Disk : $($viableDisk.DiskMappingVHD) , DiskUUID : $DiskIdentifier"
                        #find disk in Master Target that has ScsiId ends with  $DiskUUID
                        $MatchDisk = ""
                        foreach ($Disk in $TargetDiskInfo)
                            {
                                $scsiId = ($disk.Parameter | Where-Object { $_.Name -eq "ScsiId"} ).Value
                                Write-debug "`t SCSIID : $scsiId, DiskUUID : $DiskUUID" 
                                if ($scsiId.contains($DiskUUID) )
                                    {
                                    $MatchDisk = ($disk.Parameter | Where-Object { $_.Name -eq "DiskName"} ).Value
                                    write-debug "`t`t Found match . Source : $($viableDisk.DeviceLabel) , Target : $MatchDisk"
                                    $viableDisk.DiskMapping = $MatchDisk    
                                    break;
                                    }                    
                            }
...
#######################################This region builds the API request XML header and Body. And calls the API request.#                
                #region
                #call the API call to perform mapping
                $ViableDisks = $DiskArray | Where-Object { $_.DeviceLabel.contains("mapper") -eq $false}
                $PairCounter = 1
                ...
                #clone the main XML. It already includes common information like login and server IP addresses
                $temp = $CreateProtectionXML.Clone()
                
                #create XML Request body with pairing details
                #this XML output in $temp variable is to be executed via REST api  to call protection subrouting on CS server.
                foreach ($disk in $ViableDisks)
                {

                        #create Node
                        $ParameterGroupNode = $Temp.CreateElement("ParameterGroup")
                        #create Attribute
                        $ParameterGroupNodeAttribute = $Temp.CreateAttribute("Id")
                        #create AttributeText
                        $ParameterGroupNodeAttribute.Value = "Pair" + $PairCounter | Out-Null
                        $ParameterGroupNode.Attributes.Append($ParameterGroupNodeAttribute) | Out-Null

                                #Create Sub node for SourceVolume
                                    $Parameter = $Temp.CreateElement("Parameter")
                                    #create Attribute "Name"
                                    $Name = $Temp.CreateAttribute("Name")
                                    $Name.Value = "SourceVolume"
                                    #create Attribute "Value"
                                    $Value  = $Temp.CreateAttribute("Value")  
                                    $Value.Value = $disk.DeviceLabel
                                    $Parameter.Attributes.Append($name) | Out-Null
                                    $Parameter.Attributes.Append($Value) | Out-Null
                                    $parameterGroupNode.AppendChild($Parameter) | Out-Null

                                #Create Sub node for TargetVolume
                                    $Parameter = $Temp.CreateElement("Parameter")
                                    #create Attribute "Name"
                                    $Name = $Temp.CreateAttribute("Name")
                                    $Name.Value = "TargetVolume"
                                    #create Attribute "Value"
                                    $Value  = $Temp.CreateAttribute("Value")  
                                    $Value.Value = $disk.DiskMapping
                                    $Parameter.Attributes.Append($name) | Out-Null
                                    $Parameter.Attributes.Append($Value) | Out-Null
                                    $parameterGroupNode.AppendChild($Parameter) | Out-Null

                                #append to main XML Body
                                $Temp.Request.Body.FunctionRequest.ParameterGroup.AppendChild($ParameterGroupNode)  | Out-Null
                    ...
                    #Validation Block
                    #TODO : Check in InMage , how many disks are protected as against provided.
                    #In case of failure, log details and return to parent for cleanup.
                                        
                    #everything ok till here.Update object and return to main
                    #TODO : update return object and return data.
                    #packing variabled.
                    ...
                    $Object.ErrorCode = 0
                    $Object.HostIP = $SourceIP
                    $Object.Message = "Success : Initiated protection for all disks in source : $SourceIP"
                    return $Object
                    #return the main object.
                    
         }
        
         #start process for Windows Source machine
         else
         {#TODO : to be filled later in v2.
         }
